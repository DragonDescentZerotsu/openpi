    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "observation/images/table_overview": "observation.images.table_overview",
                        "observation/images/gripper_forward": "observation.images.gripper_forward",
                        "observation/images/palm_inner": "observation.images.palm_inner",
        ...
        data_transforms = _transforms.Group(
            inputs=[aero_handoff_policy.AeroHandoffInputs(model_type=model_config.model_type)],
            outputs=[aero_handoff_policy.AeroHandoffOutputs()],
        )
        model_transforms = ModelTransformFactory()(model_config)
        return dataclasses.replace(
            self.create_base_config(assets_dirs, model_config),
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_a1_piper_pipette_handoff",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
    ...
    TrainConfig(
        name="pi05_a1_piper_pipette_handoff_white_noise_1k_10k",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
        data=LeRobotAeroHandoffDataConfig(
            repo_id="aero_quest/piper_pipette_handoff_white_noise_1k",
            assets=AssetsConfig(asset_id="aero_quest/piper_pipette_handoff_white_noise_1k_pose_state"),
            state_mode=aero_handoff_policy.STATE_MODE_POSE,
        ),
        batch_size=512,
        num_workers=16,
    ...
    TrainConfig(
        name="pi05_a2_piper_aero_tip_attachment_10k",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
        data=LeRobotAeroHandoffDataConfig(
            repo_id="aero_quest/piper_aero_tip_attachment",
            assets=AssetsConfig(asset_id="aero_quest/piper_aero_tip_attachment_vision_only"),
            state_mode=aero_handoff_policy.STATE_MODE_POSE,
        ),
        batch_size=512,
        num_workers=16,
